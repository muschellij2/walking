testthat::test_that("actibase::acti_resample gives fixed answer", {
  skip_if_no_forest()
  ...
  new_data = actibase::acti_resample(data, sample_rate = 20,
                                 method = "linear",
                                 rule = 2)
  remade_data = actibase::acti_resample_to_time(new_data,
...
testthat::test_that("actibase::acti_resample with splines gives fixed", {
  skip_if_no_forest()
  ...
  data = readr::read_csv(csv_file)
  colnames(data)[colnames(data) == "UTC time"] = "time"
  data = actibase::acti_standardise_data(data)
  new_data = actibase::acti_resample(data, sample_rate = 1, method = "natural")
